...
public class MapZoomControls {
    private static class ZoomControlsHideHandler extends Handler {
            ...
            public void handleMessage(Message message) {
                    //this.zoomControls.hide();//keep the zoomcontrol available
            }
    }
    ...
    MapZoomControls(Context context, final MapView mapView) {
            ...
            this.zoomControls.setVisibility(View.VISIBLE/*GONE*/);//keep the zoomcontrol available
            ...
            mapView.addView(this.zoomControls, new MapView.LayoutParams(layoutParams));
    }
    ...
    void onMapViewTouchEvent(int action) {
            if (this.showMapZoomControls) {
                    ...
                    //Test if zoom buttons have to be activated
                    updateZoomControls(mapView.getMapPosition().getZoomLevel());
            }
    }
    
    private void updateZoomControls(int zoomLevel) {
	    // Check if the next zoom level exceeds the limits.
	    MapPosition mapPosition = new MapPosition(null, (byte) (zoomLevel+1));
	    MapViewLimits mapViewLimits = mapView.getMapViewLimits();
    	boolean zoomInEnabled = zoomLevel < this.zoomLevelMax && mapViewLimits.isZoomValid(mapPosition);
    	
    	mapPosition = new MapPosition(null, (byte) (zoomLevel - 1));
    ...
    void onZoomLevelChange(int zoomLevel) {
    	//Update zoom
    	updateZoomControls(zoomLevel);
    	mapView.moveMapBorder(zoomLevel-1);
    }
    
    void setIsZoomOutEnabled(boolean enabled) {
    	this.zoomControls.setIsZoomOutEnabled(enabled);
    }
}