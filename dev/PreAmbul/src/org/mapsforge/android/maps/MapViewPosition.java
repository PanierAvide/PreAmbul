...
public class MapViewPosition {
    ...
    public synchronized void moveMap(float moveHorizontal, float moveVertical) {
        double pixelX =
                MercatorProjection.longitudeToPixelX(this.longitude, this.zoomLevel);
        double pixelY =
                MercatorProjection.latitudeToPixelY(this.latitude, this.zoomLevel);

        MapPosition mapPosition =
                new MapPosition(new GeoPoint(
                        MercatorProjection.pixelYToLatitude(pixelY - moveVertical, this.zoomLevel),
                        MercatorProjection.pixelXToLongitude(pixelX - moveHorizontal, this.zoomLevel)),
                        this.zoomLevel);

        // Check if the new point no exceeds the limits.
        if (mapView.getMapViewLimits().isPositionValid(mapPosition)) {
            this.latitude =
                    MercatorProjection.pixelYToLatitude(pixelY - moveVertical, this.zoomLevel);
            ...
            Log.d("MapsLimits", "Limite alcanzado");
        }

    }

    synchronized void setMapCenter(GeoPoint geoPoint) {
        MapPosition mapPosition = new MapPosition(new GeoPoint(
                MercatorProjection.pixelYToLatitude(geoPoint.getLatitude(), this.zoomLevel),
                MercatorProjection.pixelXToLongitude(geoPoint.getLongitude(), this.zoomLevel)),
                this.zoomLevel);
        ...
        if (mapView.getMapViewLimits().isPositionValid(mapPosition)) {
            ...
            // FIXME Find near valid center.
            Log.e("MapsLimits", "Center not valid!");
        }
    }
    ...
    synchronized void setMapCenterAndZoomLevel(MapPosition mapPosition) {
        ...
        // Check if the new point no exceeds the limits.
        if (mapView.getMapViewLimits().isPositionValid(mapPosition)
                && mapView.getMapViewLimits().isZoomValid(mapPosition)) {
            this.latitude = MercatorProjection.limitLatitude(geoPoint.getLatitude());
            this.longitude = MercatorProjection.limitLongitude(geoPoint.getLongitude());
            this.zoomLevel = this.mapView.limitZoomLevel(mapPosition.zoomLevel);
        } else {
            // FIXME Find near valid center.
            Log.e("MapsLimits", "Center or Zoom not valid!");
        }
    }

    synchronized void setZoomLevel(byte zoomLevel) {
        MapPosition mapPosition = new MapPosition(null, zoomLevel);
        MapViewLimits mapViewLimits = mapView.getMapViewLimits();

        // Check if the new zoom level no exceeds the limits.
        if (mapViewLimits.isZoomValid(mapPosition)) {
            this.zoomLevel = this.mapView.limitZoomLevel(zoomLevel);
        } else {
        	//Zoom to a correct zoom level
        	while(!mapViewLimits.isZoomValid(mapPosition)) {
        		zoomLevel++;
        		mapPosition = new MapPosition(null, zoomLevel);
        	}
        	this.zoomLevel = this.mapView.limitZoomLevel(zoomLevel);
        }
    }
