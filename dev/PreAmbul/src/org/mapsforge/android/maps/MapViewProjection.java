...
class MapViewProjection implements Projection {
        ...
        public GeoPoint fromPixelsWithZoom(int x, int y, byte zoom) {
        if (this.mapView.getWidth() <= 0 || this.mapView.getHeight() <= 0) {
            return null;
        }
        ...
        // calculate the pixel coordinates of the top left corner
        GeoPoint geoPoint = mapPosition.geoPoint;
        double pixelX = MercatorProjection.longitudeToPixelX(geoPoint.getLongitude(), zoom);
        double pixelY = MercatorProjection.latitudeToPixelY(geoPoint.getLatitude(), zoom);
        pixelX -= this.mapView.getWidth() >> 1;
        pixelY -= this.mapView.getHeight() >> 1;

        // convert the pixel coordinates to a GeoPoint and return it
        return new GeoPoint(MercatorProjection.pixelYToLatitude(pixelY + y, zoom),
                MercatorProjection.pixelXToLongitude(pixelX + x, zoom));
    }
        ...
        public Point toPixelsWithZoom(GeoPoint in, Point out, byte zoom) {
            if (this.mapView.getWidth() <= 0 || this.mapView.getHeight() <= 0) {
            	return null;
            }
	        ...
	        // calculate the pixel coordinates of the top left corner
	        GeoPoint geoPoint = (in==null) ? in : mapPosition.geoPoint;
	        double pixelX = MercatorProjection.longitudeToPixelX(geoPoint.getLongitude(), zoom);
	        double pixelY = MercatorProjection.latitudeToPixelY(geoPoint.getLatitude(), zoom);
	        pixelX -= this.mapView.getWidth() >> 1;
	        pixelY -= this.mapView.getHeight() >> 1;
	        ...
	        if (out == null) {
	            // create a new point and return it
	            return new Point(
	                    (int) (MercatorProjection.longitudeToPixelX(in.getLongitude(), zoom) - pixelX),
	                    (int) (MercatorProjection.latitudeToPixelY(in.getLatitude(), zoom) - pixelY));
	        }
	
	        // reuse the existing point
	        out.x = (int) (MercatorProjection.longitudeToPixelX(in.getLongitude(), zoom) - pixelX);
	        out.y = (int) (MercatorProjection.latitudeToPixelY(in.getLatitude(), zoom) - pixelY);
	        return out;
        }
